...
impl FilterWorker for ZtfFilterWorker {
    ...
    async fn run(&self) -> Result<(), Box<dyn Error>> {
        ...
        for filter_id in filter_ids {
            let filter = ZtfFilter::build(filter_id, &self.filter_collection).await.unwrap();
            filters.push(filter);
        }
        ...
        for (i, filter) in filters.iter().enumerate() {
            for permission in &filter.permissions {
                if !queue_to_filter.contains_key(&format!("ztf_programid_{permission}_filter_queue")) {
                    queue_to_filter.insert(format!("ztf_programid_{permission}_filter_queue"), Vec::new());
                }
                queue_to_filter.get_mut(&format!("ztf_programid_{permission}_filter_queue")).unwrap().push(i);
            }
        }
